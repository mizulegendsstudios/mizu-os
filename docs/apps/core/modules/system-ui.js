...
export default class SystemUI {
  ...
  subscribeToAppEvents() {
    ...
    // NUEVO: Suscribirse a eventos de estado de la música
  ...
  createVideoBackground() {
    ...
    video.onerror = () => {
      ...
      // Ocultar el video si hay un error
  ...
  // NUEVO: Método para manejar eventos de control de música con prevención de múltiples clics
  handleMusicControl(eventType) {
    if (this.isProcessingMusicControl) {
      console.log(`SystemUI: Ignorando evento ${eventType} porque ya se está procesando otro control de música`);
      return;
    }
    
    this.isProcessingMusicControl = true;
    console.log(`SystemUI: Procesando evento de música: ${eventType}`);
    
    // Emitir el evento
    this.eventBus.emit(eventType);
    
    // Establecer un tiempo de espera para restablecer el estado
    setTimeout(() => {
      if (this.isProcessingMusicControl) {
        console.log(`SystemUI: Tiempo de espera agotado para ${eventType}, restableciendo estado`);
        this.isProcessingMusicControl = false;
      }
    }, 2000);
  }
